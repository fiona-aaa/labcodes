...
{
    list_entry_t *head=(list_entry_t*) mm->sm_priv;//指向用来链接记录页访问情况的链表头
    list_entry_t *entry=&(page->pra_page_link);//记录此物理页对应的虚拟页起始地址
...
_fifo_swap_out_victim(struct mm_struct *mm, struct Page ** ptr_page, int in_tick)
{
     ...
     assert(head!=le);
     struct Page *p = le2page(le, pra_page_link);//根据链表元素获得对应的Page指针p
     list_del(le);//delete
     assert(p !=NULL);
